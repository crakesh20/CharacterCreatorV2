...
function Search() {
  ...
  let moderateUser = () => {
    ...
    axios
      .patch<User>(`http://localhost:8080/users/moderate/${moderatedUser}`)
      .then((res) => {
  ...
  let search = () => {
    ...
    if (searchType == "character") {
      ...
      axios
        .get<OgChar[]>(
          `http://localhost:8080/users/search/character/${searchText}`,
    ...
    if (searchType == "user") {
      axios
        .get<User[]>(`http://localhost:8080/users/search/user/${searchText}`, {
          withCredentials: true,
        })
        .then((res) => {
          ...
          <div className="infor">
            <div className="searchTypeSelector">
              ...
              <div>
                ...
                />
                <label htmlFor="userSearchInput">Search for Character</label>{" "}
                <br />
                <input
                  id="userSearchInput"
                  type="radio"
            ...
            <button id="doSearch" onClick={search}>
              Search
            </button>
            {/* <br />
            <br /> */}
                    ...
                    return (
                      <tr key={result.userId}>
                        ...
                        <td>
                          <button
                            onClick={() => {
                              setModeratedUser(result.userId);
                            }}
                          >
